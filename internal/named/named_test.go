...
func TestNamed(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			inputQuery: "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address.city",
			inputArg: nestedStruct{
				ID: 1, Name: "Alice", Address: address{City: "Wonderland"},
			},
			expectedAt:       "SELECT * FROM users WHERE id = @p1 AND name = @p2 AND address.city = @p3",
			expectedColon:    "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address.city",
			expectedDollar:   "SELECT * FROM users WHERE id = $1 AND name = $2 AND address.city = $3",
			expectedQuestion: "SELECT * FROM users WHERE id = ? AND name = ? AND address.city = ?",
			expectedArgs:     []any{1, "Alice", "Wonderland"},
			expectError:      false,
		},
		{
			name:       "nested struct with same sub-parameter name",
			inputQuery: "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address.city OR address2.city = :address2.city",
			inputArg: nestedStructSameSubField{
				ID: 1, Name: "Alice",
				Address:  address{City: "Wonderland"},
				Address2: address{City: "Not Wonderland"},
			},
			expectedAt:       "SELECT * FROM users WHERE id = @p1 AND name = @p2 AND address.city = @p3 OR address2.city = @p4",
			expectedColon:    "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address.city OR address2.city = :address2.city",
			expectedDollar:   "SELECT * FROM users WHERE id = $1 AND name = $2 AND address.city = $3 OR address2.city = $4",
			expectedQuestion: "SELECT * FROM users WHERE id = ? AND name = ? AND address.city = ? OR address2.city = ?",
		...
		},
		{
			...
			inputQuery:       "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address",
			inputArg:         nestedStructWithPointers{ID: 1, Name: testutil.PtrTo("Alice")},
			expectedAt:       "SELECT * FROM users WHERE id = @p1 AND name = @p2 AND address.city = @p3",
			expectedColon:    "SELECT * FROM users WHERE id = :id AND name = :name AND address.city = :address",
			expectedDollar:   "SELECT * FROM users WHERE id = $1 AND name = $2 AND address.city = $3",
			expectedQuestion: "SELECT * FROM users WHERE id = ? AND name = ? AND address.city = ?",
		...
		},
		{
			...
			inputQuery:       "SELECT * FROM users WHERE id IN (:ids)",
		...
		},
		{
			...
			inputQuery:       "SELECT * FROM users WHERE name = :name AND id IN (:ids)",
			inputArg:         map[string]any{"name": "Alice", "ids": []int{4, 5, 6}},
			expectedAt:       "SELECT * FROM users WHERE name = @p1 AND id IN (@p2,@p3,@p4)",
			expectedColon:    "SELECT * FROM users WHERE name = :name AND id IN (:ids,:ids,:ids)",
			expectedDollar:   "SELECT * FROM users WHERE name = $1 AND id IN ($2,$3,$4)",
			expectedQuestion: "SELECT * FROM users WHERE name = ? AND id IN (?,?,?)",
